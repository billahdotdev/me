  ...
  return (
    ...
    >
      ...
      {activePopup && (
        ...
        >
          <div
            className={`bg-surface border border-border rounded-2xl p-10 transition-all duration-400 shadow-2xl backdrop-blur-xl ${
              isClosing ? "opacity-0 scale-90" : "opacity-100 scale-100"
            } ${popupPosition.direction === "left" ? "origin-right" : "origin-left"}`}
            onClick={(e) => e.stopPropagation()}
            style={activePopup ? getPopupStyle() : {}}
          >
            <button
              className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center transition-colors hover:bg-surface-hover text-muted-foreground hover:text-foreground"
              onClick={closePopup}
            ...
            <div className="text-center">
              <img
                ...
                alt="Masum Billah"
                className="w-20 h-20 rounded-full mx-auto mb-5 border-2 border-border shadow-lg"
              />
              <h3 className="text-xl font-medium mb-3 text-foreground">{popupContent.title}</h3>
              {popupContent.subtitle && (
                <p className="text-sm mb-5 italic text-muted-foreground">{popupContent.subtitle}</p>
              )}
              <p className="text-sm mb-6 px-3 py-2 rounded-lg font-mono bg-surface-hover text-muted-foreground">
                {popupContent.email}
              </p>
              <button
